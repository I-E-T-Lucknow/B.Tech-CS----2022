...
import time
from datasets import Attirefitin, AttirefitinDataLoader
from networks import SegGenerator, GMM, ALIASGenerator
from utils import gen_noise, load_checkpoint, newshow, save_images, concat, show, newshow
import warnings
...
def test(opt, seg, gmm, alias):
    up = nn.Upsample(size=(opt.load_height, opt.load_width), mode='bilinear', align_corners=True)
    gauss = tgm.image.GaussianBlur((15, 15), (3, 3))
    gauss.cpu()

    test_dataset = Attirefitin(opt)
    test_loader = AttirefitinDataLoader(opt, test_dataset)
    ...
    with torch.no_grad():
        for i, inputs in enumerate(test_loader.data_loader):  
            ...
            c_masked_down = F.interpolate(c * cm, size=(256, 192), mode='bilinear',align_corners=True)
            ...
            for img_name, c_name in zip(img_names, c_names):
                ...
                image_parse = img_name.split('.')[-2]

                im1 = os.path.join('D:/Final Year Project/Attire_Fit_In/datasets/test/image',img_name).replace('\\','/')
                im2 = os.path.join('D:/Final Year Project/Attire_Fit_In/datasets/test/cloth',c_name).replace('\\','/')
                im5 = os.path.join('D:/Final Year Project/Attire_Fit_In/datasets/test/image-parse',f'{image_parse}.png').replace('\\','/')
                print(im5)
                im6 = os.path.join('D:/Final Year Project/Attire_Fit_In/datasets/test/openpose-img',f'{image_parse}_rendered.png').replace('\\','/')
                print(im6)

                im3 = os.path.join(os.path.join(opt.save_dir, opt.name), '{}_{}'.format(img_name.split('_')[0], c_name)).replace('\\','/')
                im4 = os.path.join(os.path.join(opt.save_dir, opt.name), 'concat{}_{}'.format(img_name.split('_')[0], c_name)).replace('\\','/')

            save_images(output, unpaired_names, os.path.join(opt.save_dir, opt.name))
            try:
                concat(im1,im2,im5,im6,im3,im4)
            except Exception as e:
                pass
    
            ##CONCAT
            # im1 = os.path.join('D:/Final Year Project/Attire_Fit_In/datasets/test/image',img_names[i]).replace('\\','/')
            # im2 = os.path.join('D:/Final Year Project/Attire_Fit_In/datasets/test/cloth',c_name).replace('\\','/')
            # im3 = os.path.join(os.path.join(opt.save_dir, opt.name), '{}_{}'.format(img_name.split('_')[0], c_name)).replace('\\','/')
            # im4 = os.path.join(os.path.join(opt.save_dir, opt.name), 'concat{}_{}'.format(img_name.split('_')[0], c_name)).replace('\\','/')
            ...
            filen = im4.split('/')[-1]
            fpath = f'D:/Final Year Project/Attire_Fit_In/results/new/{filen}'

            nfile = im3.split('/')[-1]
            newf = f'D:/Final Year Project/Attire_Fit_In/results/new/{nfile}'
